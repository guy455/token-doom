# Changelog

## v1.0

- Play in the browser: PrBoom + SDL compiled to WebAssembly, with mouse aim,
  WASD, and sound. No install.
- Desktop builds for Windows and macOS (bring your own `doom.wad`).
- Monsters renamed after the risks Token hunts; weapons are Token capabilities,
  labeled on the gun.
- HUD relabeled: TOKENS, POSTURE, TOOLS, TRUST.
- Player face is the Token logo: grins on a pickup, reddens on a rampage, wears
  shades in god mode, and cracks as posture drops.
- Name tags read correctly from every angle (mirror-shared sprite rotations are
  split so the text never flips).
- Cheats: `iddqd` (Enzo Mode), `idkfa` (Full Inventory), `idclev11`-`idclev19`
  (Change Environment).
